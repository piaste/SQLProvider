        ...
        rootType.AddMembersDelayed (fun () -> 
            [ 
              ...
              let crossTargetParameterCombinations = [
                    ...
                    [transopt], (fun args ->
                                let runtimeAssembly, runtimeConStr, contextSchemaPath = getConnectionInfo args true
                                <@@ SqlDataContext(rootTypeName, %%runtimeConStr, dbVendor, resolutionPath, %%referencedAssemblyExpr, runtimeAssembly, owner, caseSensitivity, tableNames, "", odbcquote, sqliteLibrary, %%runtimeConStr, None, %%defaultSelectOperations) :> ISqlDataContext @@>);
                    [cmdTimeout], (fun args ->
                                let runtimeAssembly, runtimeConStr, contextSchemaPath = getConnectionInfo args true

                                <@@ SqlDataContext(rootTypeName, %%runtimeConStr, dbVendor, resolutionPath, %%referencedAssemblyExpr, runtimeAssembly, owner, caseSensitivity, tableNames, contextSchemaPath, odbcquote, sqliteLibrary, %%defaultTransactionOptionsExpr, (Some %%runtimeConStr), %%defaultSelectOperations) :> ISqlDataContext @@>);
                    [transopt;cmdTimeout], (fun args ->
                                let runtimeAssembly, runtimeConStr, contextSchemaPath = getConnectionInfo args true

                                <@@ SqlDataContext(rootTypeName, %%runtimeConStr, dbVendor, resolutionPath, %%referencedAssemblyExpr, runtimeAssembly, owner, caseSensitivity, tableNames, "", odbcquote, sqliteLibrary, %%runtimeConStr, (Some %%args.[1]), %%defaultSelectOperations) :> ISqlDataContext @@>);
                    [selectOperations], (fun (args:Expr list) ->
                                let runtimeAssembly, runtimeConStr, contextSchemaPath = getConnectionInfo args true

                                <@@ SqlDataContext(rootTypeName, %%runtimeConStr, dbVendor, resolutionPath, %%referencedAssemblyExpr, runtimeAssembly, owner, caseSensitivity, tableNames, contextSchemaPath, odbcquote, sqliteLibrary, %%defaultTransactionOptionsExpr, None, %%runtimeConStr) :> ISqlDataContext @@>);
